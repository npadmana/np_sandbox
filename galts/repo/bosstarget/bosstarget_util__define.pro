...
;
;	Dependencies:
;		This code dependends on sdssidl in addition to the BOSS software stack.
;
...
;
; typical steps after making collatefile for qso
; note this is all in the postprocess_collated method, but
; the density tune you probably want to do in parallel...
;   split_suppz, 'main012'
;   subreg_density_tune, 'main011', 'ngcgood', extra='nosuppz'
;   subreg_density_tune, 'main011', 'sgc', extra='nosuppz'
;   run_plot_subreg_density_qso, 'main011', extra='nosuppz' 
;   make_subreg_density_qso_html, 'main011', extra='nosuppz'
;
;
; Then some analysis:
;   ::plot_subreg_density_byrun
;	::compare_densities
...
function bosstarget_util::init
	return, 1
end

pro bosstarget_util::postprocess_collated, run
    self->split_suppz, run
    self->subreg_density_tune, run, 'ngcgood', extra='nosuppz'
    self->subreg_density_tune, run, 'sgc', extra='nosuppz'
    self->run_plot_subreg_density_qso, run, extra='nosuppz' 
    self->make_subreg_density_qso_html, run, extra='nosuppz'
end

pro bosstarget_util::split_suppz, target_run
    bt=obj_new('bosstarget')
    t=bt->read_collated('qso',target_run, status_struct=ss)

    print,'Total objects:',n_elements(t),f='(a,i0)'
    suppz_flag = sdss_flagval('boss_target1', 'qso_known_suppz')
    wsuppz=where((t.boss_target1 and suppz_flag) ne 0, n_suppz, $
                  comp=wnosuppz, ncomp=n_nosuppz)
    
    if n_suppz ne 0 then begin
        ; remove this flag!
        t[wsuppz].boss_target1 -= suppz_flag

        outf=bt->target_file('qso',target_run,/collate,extra_name='suppz')
        tt = t[wsuppz]
        tt.boss_target1 = 0
        est={sourcetype:'qsoknownsuppz',priority:0}
        st=create_struct(tt[0], est)
        st=replicate(st, n_suppz)
        struct_assign, tt, st, /nozero
        print,'writing ',n_suppz,' to file ',outf,format='(a,i0,a,a)'
        mwrfits, ss, outf, /create
        mwrfits, st, outf
    endif

    if n_nosuppz ne 0 then begin
        outf=bt->target_file('qso',target_run,/collate,extra_name='nosuppz')
        print,'writing ',n_nosuppz,' to file ',outf,format='(a,i0,a,a)'
        tt=t[wnosuppz]
        mwrfits, ss, outf, /create
        mwrfits, tt, outf
    endif
...
pro bosstarget_util::subreg_density_tune, target_run, chunks, old=old, extra_name=extra_name

	target_density = 40.0
    std = '40'

    ; note: we usually want 'ngcgood', and 'sgc' (not 'sgcgood')
    chunks_tune = chunks
    if chunks[0] eq 'ngc' then begin
		fend='-ngc'+std+'.fits'
    endif else if chunks[0] eq 'sgc' then begin
		fend='-sgc'+std+'.fits'
    endif else if chunks[0] eq 'ngcgood' then begin
		fend='-maskngc'+std+'.fits'
	endif else if chunks[0] eq 'sgcgood' then begin
		fend='-masksgc'+std+'.fits'
	endif else if chunks[0] eq 'sgc-small1' then begin
		fend='-masksgc1'+std+'.fits'
	endif else begin
		message,'bad chunk: '+string(chunks)
	endelse

	maxres=3

	bt=obj_new('bosstarget')
	tfile=bt->target_file('qso',target_run,/collate,extra_name=extra_name)
	new_target_file=repstr(tfile,'.fits',fend)
	print,'will write to new target file: ',new_target_file
	if n_elements(str) eq 0 or n_elements(status_struct) eq 0 then begin
		str=bt->read_collated('qso',target_run, extra_name=extra_name, $
                              status_struct=status_struct)
	endif

    if keyword_set(old) then begin
        nn_value_all = str.nn_value
    endif else begin
        ; use the ukidss value when we can.  Note ordering is not
        ; perfect, so in pixels only partial coverage of ukidss
        ; exists, this is also not perfect
        nn_value_all = str.nn_value_with_ed
        w=where(str.ukidss_matched ne 0, nw)
        if nw ne 0 then begin
            splog,'Found ',nw,' with ukidss',format='(a,i0,a)'
            nn_value_all[w] = str[w].nn_value_with_ed_ukidss
        endif
    endelse

	; memory managed internally by the cache mechanism, do not destroy
	self->bosstile_sub_regions, chunks_tune, maxres, $
	...
	extra_flagnames=['qso_known_midz','qso_first_boss','qso_known_suppz']
	extra_flags = $
    ...
    if chunks[0] eq 'ngcgood' then begin
        print,'Checking core against stricter mask ngcgood2'
        corepoly = bt->chunk_polygon_read('ngcgood2',/verbose)
        inwin2 = is_in_window( corepoly, ra=str.ra, dec=str.dec )

        print,'removing ngcgood2 masked core'

        w=where( (inwin eq 1) and (inwin2 eq 0) $
                  and (str.boss_target1 and coreflag) ne 0,nw)
        if nw ne 0 then begin
            print,'  removing ',nw

            str[w].boss_target1 -= coreflag
        endif
        destruct_polygon, corepoly
    endif
    ...
    wsuppz=where((str.boss_target1 and sdss_flagval('boss_target1','qso_known_suppz')) ne 0)
    help,wsuppz
    ...
    addst = {count:         vl, $
             ...
             l:             vd, $
             b:             vd}
    density_struct = struct_addtags(regions, addst)
	...
	for i=0L,nreg-1 do begin
		...
		if density_struct[i].core_density lt target_density then begin
			...
			if nthis_bonus ne 0 then begin
				...
				nn_value = nn_value_all[winthis_bonus]
...
pro bosstarget_util::main008_edcore_loose, run
    ; this runs through and selects the qsoed in a range of prob
    ; > 0.3
    bt=obj_new('bosstarget')
    ...
    for camcol=1,6 do begin
        ...
        f = bt->target_file('qso','main008',run,301,camcol)
        fnew = bt->target_file('qso','main008',run,301,camcol,extra_name='edloose')

        str = bt->read('qso','main008',run,301,camcol,status=status_struct)

        ; set new edcore and qso_core main

        w=where((str.boss_target1 and coreflag) ne 0, nw)
        print,'Number in old core: ',nw

        w=where(str.qsoed_prob_core gt probval, nw)
        print,f='("Number > ",f0,": ",i0)',probval,nw

        if nw gt 0 then begin
            outst = replicate({ra:0d, dec:0d, qsoed_prob_core:0.0}, nw)

            outst.ra = str[w].ra
            outst.dec = str[w].dec
            outst.qsoed_prob_core = str[w].qsoed_prob_core

            if fexist(fnew) then begin
                message,'file exists'
            endif
            print,'writing file: ',fnew
            mwrfits, outst, fnew, /create
        endif
    endfor
end


pro bosstarget_util::main008_edcore_kludge, run
    ; overall plan
    ; create pbs to run this
    ;   main008_edcore_kludge, run, thresh, extra_name
    ; on all runs.
    ; Also pbs to gather them.
    ; then tune the subreg stuff
    ; then make the density map plots
    ;
    ; A worked example:
    ;
    if n_params() lt 1 then begin
        message,'usage: main008_edcore_kludge, run'
    endif
    ; redefine the core
    bt=obj_new('bosstarget')
    ...
    pars = bq->pars()
    thresh = pars.ed_thresh_core
    extra_name = 'edfinal'

    coreflag = sdss_flagval('boss_target1','qso_core_main')
    edflag = sdss_flagval('boss_target1','qso_core_ed')
    for camcol=1,6 do begin

        f = bt->target_file('qso','main008',run,301,camcol)
        fnew = bt->target_file('qso','main008',run,301,camcol,extra_name=extra_name)
        print,f
        print,fnew
        ...
        ; unset old ExD core
        w=where((str.boss_target1 and edflag) ne 0, nw)
        print,'Number with old ExD core: ',nw
        if nw ne 0 then begin
            str[w].boss_target1 -= edflag
        endif

        ; unset old qso_core_main (from like)
        w=where((str.boss_target1 and coreflag) ne 0, nw)
        print,'Number in old core: ',nw
        ...
        ; set new edcore and qso_core main
        w=where(str.qsoed_prob_core gt thresh, nw)
        print,'Applying new ExD thresh: ',thresh
        print,'Number passing new ExD core cut: ',nw
        if nw gt 0 then begin
            str[w].boss_target1 += edflag
            str[w].boss_target1 += coreflag
        endif
        ...
        w=where((str.boss_target1 and coreflag) ne 0, nw)
        print,'Number in new core: ',nw

        status_struct=create_struct(status_struct, {extra_name:extra_name})
        status_struct.ed_thresh_core = thresh
		status_struct.bosstarget_v += ' : '+bosstarget_version()
        mwrfits, status_struct, fnew, /create
        mwrfits, str, fnew
...
function bosstarget_util::substitute_core, corestruc, bonusstruc, coreflags

	; Match a structure that corresponds to CORE objects with a structure that
	; corresponds to BONUS objects. Output a new structure that is the BONUS
	; structure but with the CORE structure written over the top where coreflags
	; are set
	; INPUTS corestruc: The structure that corresponds to the core structure
	;	bonusstrun: The structure that corresponds to the bonus structure
	;	coreflags: A list of flags that denote what is retained as core (this can be several elements)
	; OUTPUTS a structure with bonusstruc overwritten by corestruc where appropriate
	;
	; NOTES: Main use will be if we run bosstarget_qso__define twice, once for the core
	;	and once for the bonus. We will probably have to do this as the bonus
	;	becomes complex (i.e. the core will be constructed off single-epoch data, the bonus
	;	off multi-epoch, multiwavelength data).
	;
	; Written by Adam D. Myers, UIUC, April 20, 2010
	
	; ADM check core and bonus structure have exactly the same format
	; ADM this can be achieved by checking they have the same number of tags
	; ADM and that the tags are ordered in the same way

	coretags = tag_names(corestruc)
	bonustags =  tag_names(bonusstruc)
	ncoretags = n_elements(coretags)
	nbonustags =  n_elements(bonustags)
	
	if min(coretags eq bonustags) ne 1 then message, 'Input structures differ'
	if ncoretags ne nbonustags then message, 'Input structures differ'

	;ADM Now we know the input structures are the same
	;ADM make a structure that is only core objects from the input corestruc
	;ADM first make a bitmask that's gt 0 where coreflags[0] is a bit
	truecore = (coreflags[0] and corestruc.boss_target1)

	;ADM now loop through rest of coreflags (if it exists) and add other bits
	for i = 1, n_elements(coreflags)-1 do begin
		truecore += (coreflags[i] and corestruc.boss_target1)
	endfor

	;ADM now truecore is non-zero for core objects
	tcw = where(truecore ne 0, cnt)
	if cnt gt 0 then begin
		truecorestruc = corestruc[tcw]
	endif else begin
		message, 'Nothing in corestruc is defined as core by coreflags'
	endelse

	;ADM match the core objects to the bonus structure on the unique photoid
	tcid = sdss_photoid(truecorestruc)
	bid = sdss_photoid(bonusstruc)
	match, tcid, bid, m1, m2, /sort

	;ADM Now overwrite the bonus structure where the core struc has a coreflag
	;ADM and return this new structure
	donestruc = bonusstruc
	donestruc[m2] = truecorestruc[m1]	

	return, donestruc

end

function bosstarget_util::density_tune_file, target_run, chunks, maxres
...
function bosstarget_util::density_tune_read, target_run, chunks, maxres, hdr=hdr
...
function bosstarget_util::run_rerun_camcol_index, run, rerun, camcol
	rstr = string(run,f='(i0)')
	rrstr = string(rerun,f='(i0)')
	cstr = string(camcol,f='(i0)')
	stringind = rstr+'-'+rrstr+'-'+cstr
	return, stringind
end
pro bosstarget_util::get_unique_run_rerun_camcol, $
		run, rerun, camcol, urun, urerun, ucamcol, uind=uind
	stringind = self->run_rerun_camcol_index(run, rerun, camcol)
	uind = rem_dup(stringind)
...
function bosstarget_util::get_lups, objs, err=err, deredden=deredden
	if n_tags(objs) eq 0 then begin
		help,objs
		message,'You must input a structure',/inf
		message,'Usage: lups=bq->get_lups(objs, err=, /deredden)'
	endif

	if keyword_set(deredden) then begin
		flux=self->deredden(objs.psfflux, objs.extinction)
		if arg_present(err) then begin
			ivar=self->deredden_error(objs.psfflux_ivar, objs.extinction)
		endif
	endif else begin
		flux = objs.psfflux
		if arg_present(err) then begin
			ivar = objs.psfflux_ivar
		endif
	endelse

	luptitudes = self->flux2lups(flux)
	if arg_present(err) then begin
		err = self->ivar2lupserr(ivar, flux)
	endif
...
function bosstarget_util::flux2lups, nmgy, band=band
	; send band= if you sent a single band
	bval=[1.4D, 0.9D, 1.2D, 1.8D, 7.4D]

	if n_elements(band) ne 0 then begin
		; a single band was sent
		lups = self->flux2mags(nmgy, bval[band])
	endif else begin
		lups = double(nmgy)
		for iband=0L,4 do begin
			lups[iband,*] = self->flux2mags(nmgy[iband,*], bval[iband])
		endfor
	endelse
...
function bosstarget_util::ivar2lupserr, ivar, flux, band=band
	; send band= if you sent a single band
	bval=[1.4D, 0.9D, 1.2D, 1.8D, 7.4D]

	if n_elements(band) ne 0 then begin
		; a single band was sent
		lupserr = self->ivar2magerr(ivar, flux, bval[band])
	endif else begin
		lupserr = double(flux)
		for iband=0L,4 do begin
			lupserr[iband,*] = self->ivar2magerr(ivar[iband,*], flux[iband,*], bval[iband])
		endfor
	endelse
...
function bosstarget_util::flux2mags, f, b
	...
	IF n_elements(f) GT 1 THEN begin
		m = -a*(ASINH(5.0*f/b)+ln10_min10 + ALOG(b)) 
	endif else begin
		m = -a*(ASINH(5.0*[f]/b)+ln10_min10 + ALOG(b))    
	endelse
...
function bosstarget_util::ivar2magerr, ivar, f, b
	...
	; big number indicates infinite error
	IF n_zero NE 0 THEN dm[zero_ind] = 1.0e8 
...
;docstart::bosstarget_util::deredden
;PURPOSE
;	deredden a flux
;SYNTAX
;	bq = obj_new('bosstarget_qso')
;	dered=bq->deredden(flux, extinction)
;INPUTS
;	flux: the linear flux
;	extinction: the extinction (i.e. from the tsobj tag)
;OUTPUT
;	dered: the dereddened flux
;
;Written by J. Hennawi
;Documenation Header by R. da Silva
; 13-Jan-2009: Moved into class file, formatting.  Erin Sheldon, BNL
;docend::bosstarget_util::deredden

function bosstarget_util::deredden, flux, extinction
...
;docstart::bosstarget_util::deredden_error
;PURPOSE
;	to scale the error when correcting for extinction
;SYNTAX
;	bq = obj_new('bosstarget_qso')
;	corrected_ivar=bq->deredden_error(ivar, extinction)
;INPUTS
;	ivar: the inverse variance
;	extinction: the extinction
;OUTPUT
;	corrected_ivar: the corrected inverse variance
;
;Written by J. Hennawi
;Documentation Header by R. da Silva
; 13-Jan-2009: Moved into class file, formatting.  Erin Sheldon, BNL
;docend::bosstarget_util::deredden_error

function bosstarget_util::deredden_error, ivar2, extinction
...
;docstart::bosstarget_util::deredden_assign
;PURPOSE
;	To properly deredden objects
;SYNTAX
;	bq = obj_new('bosstarget_qso')
;	objs=bq->deredden_assign(stars)
;INPUTS
;	stars: any structure of objects that have the following tags
;		-PSFFLUX
;		-PSFFLUX_IVAR
;		-EXTINCTION
;OUTPUT
;	objs: the same structure as stars but now the psffluxes have 
;		dereddened
;ROUTINES CALLED
;	deredden
;	deredden_error
...
;Documenation Header by R. da Silva
; 13-Jan-2009: Moved into class file, formatting.  Erin Sheldon, BNL
;docend::bosstarget_qso::deredden_assign



function bosstarget_util::deredden_assign, stars

	stars1 = stars

	; deredden the fluxes and inverse variances
	stars1.psfflux[0] =  self->deredden(stars.psfflux[0], stars.extinction[0])
	stars1.psfflux[1] =  self->deredden(stars.psfflux[1], stars.extinction[1])
	stars1.psfflux[2] =  self->deredden(stars.psfflux[2], stars.extinction[2])
	stars1.psfflux[3] =  self->deredden(stars.psfflux[3], stars.extinction[3])
	stars1.psfflux[4] =  self->deredden(stars.psfflux[4], stars.extinction[4])

	stars1.psfflux_ivar[0] = $
		self->deredden_error(stars.psfflux_ivar[0], stars.extinction[0])
	stars1.psfflux_ivar[1] = $
		self->deredden_error(stars.psfflux_ivar[1], stars.extinction[1])
	stars1.psfflux_ivar[2] = $
		self->deredden_error(stars.psfflux_ivar[2], stars.extinction[2])
	stars1.psfflux_ivar[3] = $
		self->deredden_error(stars.psfflux_ivar[3], stars.extinction[3])
	stars1.psfflux_ivar[4] = $
		self->deredden_error(stars.psfflux_ivar[4], stars.extinction[4])
	return, stars1
end
...
function bosstarget_util::qsoselectflag, name
; 27-May-2009: Only ultimately want sdss_flagval to return bosstarget yes/no, so I wrote an
;		internal selection bitmask to track the possible qso selections
;		Adam D. Myers, UIUC

	case strlowcase(name) of

		'qso_kde_core': return, 2L^0
		'qso_chi2_core': return, 2L^1
		'qso_kde_bonus': return, 2L^2
		'qso_chi2_bonus': return, 2L^3
		else: begin
			on_error,2
...
pro bosstarget_util::check_file_exists, filename, deredden=deredden
; 15-May-2009: Function to check if a file exists and return an error message
;		Adam D. Myers, UIUC
	if not file_test(filename) then begin
		message,'faint labels file '+filename+' not found'
	endif
end
...
function bosstarget_util::orflags, flags, types
...
function bosstarget_util::andflags, flags, types, notset=notset
	; sdss_flagval sums an array of flags
...
function bosstarget_util::get_field_stats, struct
...
function bosstarget_util::btlogic, str, target_flags_or_type, $
        without_notflags=without_notflags, $
		all=all, $
		only=only, $
		fixup=fixup, $
		objc_type=objc_type, $
	...
	if strmatch(target_flags_or_type[0], 'qso*') then begin
        ...
        if not keyword_set(without_notflags) then begin
		...
		if target_flags_or_type[0] eq 'lrg' then begin
			...
			target_flags = ['gal_loz','gal_cmass','gal_cmass_sparse']
		endif else begin
			target_flags = target_flags_or_type
		endelse
        if not keyword_set(without_notflags) then begin
            notflags += sdss_flagval('boss_target1','sdss_known')
        endif
	...
	if not keyword_set(silent) then begin
		splog,'Selecting target types "',target_flags,'"';, form='(a,a,a)'
		if notflags[0] ne 0 then begin
			splog,'selecting against : ',notflags
		endif
	endif
...
function bosstarget_util::btselect, str, target_flags_or_type, $
        chunk=chunk, $
        bounds=bounds, $
        without_notflags=without_notflags, $
		objc_type=objc_type, $
    ...
    if n_elements(chunk) ne 0 then begin
        splog,'Trimming to chunk: ',chunk
        bt=obj_new('bosstarget')
        w=where(bt->is_in_chunk(chunk, str[wtargets].ra, str[wtargets].dec, bounds=bounds),count)
...
function bosstarget_util::observed_platedir
	dir=getenv('BOSS_TARGET')
	return, path_join(dir, 'observed-plates')
end
function bosstarget_util::observed_platefile, chunk=chunk, orig=orig
...
function bosstarget_util::read_observed_plates, chunk=chunk, orig=orig
...
pro bosstarget_util::observed_plates_addarea, chunk
...
function bosstarget_util::mmt_plateinfo
...
function bosstarget_util::mmt_match_dir, type, all=all
	boss_target = getenv("BOSS_TARGET")
...
function bosstarget_util::mmt_match_psdir, type, all=all
	dir = '~/public_html/bosstarget/density/mmt'
	return, dir
end
function bosstarget_util::mmt_match_file, type, index, all=all, targets=targets
...
pro bosstarget_util::mmt_combine_and_target, type, str, all=all
...
pro bosstarget_util::mmt_density, target_flags, str=str, bin=bin
...
pro bosstarget_util::make_run_html_run, reload=reload
...
function bosstarget_util::bosstile_radius
	return, 1.49d
end
...
function bosstarget_util::circle_poly, ra, dec, tilerad
...
function bosstarget_util::circle_poly_area, ra, dec, geom, tilerad=tilerad;, total_area=total_area
...
function bosstarget_util::tile_area_old, ra, dec, geom, tilerad=tilerad
...
pro bosstarget_util::_random_radec_circle, ra, dec, R, rand_ra, rand_dec
	; R in degrees
...
function bosstarget_util::bosstile_read, chunk
...
pro bosstarget_util::bosstile_match_poly, $
		ra, dec, chunks, $
...
pro bosstarget_util::make_inwindow, target_type, target_run, chunk, $
...
pro bosstarget_util::make_fullcache_and_inwindow, target_type, target_run, chunk, $
...
pro bosstarget_util::bosstile_match, struct, mstruct, mtile, distances, $
...
function bosstarget_util::bosstile_chunk_bounds_polygon_file, chunk
...
function bosstarget_util::bosstile_chunk_polygon_file, chunk
...
function bosstarget_util::bosstile_poly_read, chunk, bounds=bounds, verbose=verbose
...
function bosstarget_util::bosstile_is_in_window, ra, dec, chunk, bounds=bounds
...
function bosstarget_util::bosstile_inwindow, ra, dec, chunk, bounds=bounds, count=count
	if (n_elements(ra) eq 0 $
		  or n_elements(dec) eq 0 $
		  or n_elements(chunk) eq 0) then begin
		message,'usage: inwindow=eb->'+$
...
function bosstarget_util::bosstile_mean_density, ra, dec, chunk, bounds=bounds
	if (n_elements(ra) eq 0 $
		  or n_elements(dec) eq 0 $
		  or n_elements(chunk) eq 0) then begin
...
function bosstarget_util::bosstile_density, ra, dec, chunks, $
		getmean=getmean, $
		$
		tiles=tiles, $
...
function bosstarget_util::density_cache_file, chunks, maxres, nra
	boss_target = getenv('BOSS_TARGET')
...
pro bosstarget_util::density_cache_write, $
...
function bosstarget_util::density_cache_read, chunks, maxres, nra
...
pro bosstarget_util::cache_bosstile_subreg_density, $
		...
		if nw gt 0 then begin
			...
			for i=0L,nreg-1 do begin
				...
				inwin = is_in_window( $
					...
					ra=raw, dec=decw)
				w2=where(inwin, nw2)
				print,'   ',i+1,'/',nreg,': ',nw2,form='(a,i0,a,i0,a,i0)'
...
function bosstarget_util::bosstile_subreg_density, ra, dec, chunks, $
		maxres=maxres, $
		polygons=polygons, $
		force=force
...
pro bosstarget_util::bosstile_denscut_area, method, str=str, dens=dens
	...
	sra= self->shiftra(dens.dstruct.ra, 180)
...
function bosstarget_util::get_ngc_large_thresholds
...
pro bosstarget_util::do_plot_subreg_density_2010_03_03d60, method, goal, str=str, $
...
pro bosstarget_util::make_subreg_density_qso_html, target_run, extra_name=extra_name
    ...
    for i=0L, n_elements(chunks)-1 do begin
        ...
        if chunk eq 'ngc' then begin
            corechunk='ngcgood2'
            bonuschunk='ngcgood'
            allchunk='ngcgood'
            htmlname=ename+'qso-ngc.html'
            extra=ename+'maskngc40'
        endif else if chunk eq 'sgc' then begin
            corechunk='sgc'
            bonuschunk='sgc'
            allchunk='sgc'
            htmlname=ename+'qso-sgc.html'
            extra=ename+'sgc40'
        endif else begin
            message,'chunk ngc or sgc'
        endelse

        fitsname='bosstarget-qso-'+target_run+'-collate-'+extra+'.fits'
        
        fits_address='../'+fitsname
        html_address ='http://data.sdss3.org/sas/bosswork/groups/boss/target/'+target_run+'/plots/'+htmlname

        psdir=filepath( $
            root=getenv('BOSS_TARGET'), subdir=[target_run[0]],'plots')
        htmlfile = filepath(root=psdir, htmlname)
        print,'address: ',html_address
        print,'Writing to html file: ',htmlfile
        openw, lun, htmlfile, /get

        printf,lun,'<h1>Release '+target_run+' '+chunk+'</h1>'
        printf,lun,'<h2>Target file</h2>'
        printf,lun,'<a href="'+fits_address+'">'
        printf,lun,'    '+fitsname
        printf,lun,'</a>'

        printf,lun,'<h2>Plots</h2>'
        printf,lun,'<ul>'
        printf,lun,'    <li><a href="#core">Core Targets</a></li>'
        printf,lun,'    <li><a href="#corebonus">Core+Bonus Targets</a></li>'
        printf,lun,'    <li><a href="#all">All Targets</a></li>'
        printf,lun,'</ul>'

        printf,lun,'<h2 id="core">QA plots for Core Targets only</h2>'

        printf,lun, '<h3>Heat map at the tuning resolution</h3>'
        printf,lun,'<img src="'+target_run+'-qso-'+extra+'-chunk'+corechunk+'-qso-core-main-densmap-hammer-maxres3.png"><br>'

        printf,lun,'<h3>Heat map at the higher resolution</h3>'
        printf,lun,'<img src="'+target_run+'-qso-'+extra+'-chunk'+corechunk+'-qso-core-main-densmap-hammer-maxres5.png"><br>'

        printf,lun,'<h3>Density histogram in the high-res heat map.</h3>'
        printf,lun,'<img src="'+target_run+'-qso-'+extra+'-chunk'+corechunk+'-qso-core-main-denshist-maxres5.png"><br>'



        printf,lun,'<h2 id="corebonus">QA plots for Core+Bonus Targets only</h2>'

        printf,lun, '<h3>Heat map at the tuning resolution</h3>'
        printf,lun,'<img src="'+target_run+'-qso-'+extra+'-chunk'+bonuschunk+'-qso-core-main-qso-bonus-main-densmap-hammer-maxres3.png"><br>'

        printf,lun,'<h3>Heat map at the higher resolution</h3>'
        printf,lun,'<img src="'+target_run+'-qso-'+extra+'-chunk'+bonuschunk+'-qso-core-main-qso-bonus-main-densmap-hammer-maxres5.png"><br>'

        printf,lun,'<h3>Density histogram in the high-res heat map.</h3>'
        printf,lun,'<img src="'+target_run+'-qso-'+extra+'-chunk'+bonuschunk+'-qso-core-main-qso-bonus-main-denshist-maxres5.png"><br>'


        printf,lun,'<h2 id="all">QA plots for All Targets</h2>'

        printf,lun,'<h3>Heat map at the tuning resolution</h3>'
        printf,lun,'<img src="'+target_run+'-qso-'+extra+'-chunk'+allchunk+'-qso-core-main-qso-bonus-main-qso-first-boss-qso-known-midz-densmap-hammer-maxres3.png"><br>'

        printf,lun,'<h3>Heat map at the higher resolution</h3>'
        printf,lun,'<img src="'+target_run+'-qso-'+extra+'-chunk'+allchunk+'-qso-core-main-qso-bonus-main-qso-first-boss-qso-known-midz-densmap-hammer-maxres5.png"><br>'

        printf,lun,'<h3>Density histogram in the high-res heat map.</h3>'
        printf,lun,'<img src="'+target_run+'-qso-'+extra+'-chunk'+allchunk+'-qso-core-main-qso-bonus-main-qso-first-boss-qso-known-midz-denshist-maxres5.png"><br>'
...
pro bosstarget_util::make_subreg_density_lrg_html, target_run
    ...
    for i=0L, n_elements(chunks)-1 do begin
        ...
        if chunk eq 'ngc' then begin
            region='ngc'
            htmlname='lrg-ngc.html'
        endif else if chunk eq 'sgc' then begin
            region='sgc'
            htmlname='lrg-sgc.html'
        endif else begin
            message,'chunk ngc or sgc'
        endelse

        fitsname='bosstarget-lrg-'+target_run+'-collate.fits'
        ...
        printf,lun,'<h1>Release '+target_run+' '+region+'</h1>'
        printf,lun,'<h2>Target file</h2>'
        printf,lun,'<a href="http://data.sdss3.org/sas/bosswork/groups/boss/target/'+target_run+'/'+fitsname+'">'
        printf,lun,'    '+fitsname
        printf,lun,'</a>'


        printf,lun,'<h2>QA plots</h2>'

        printf,lun, '<h3>Heat map at low resolution</h3>'
        printf,lun,'<img src="'+target_run+'-lrg-chunk'+chunk+'-lrg-densmap-hammer-maxres3.png"><br>'

        printf,lun,'<h3>Heat map at higher resolution</h3>'
        printf,lun,'<img src="'+target_run+'-lrg-chunk'+chunk+'-lrg-densmap-hammer-maxres5.png"><br>'

        printf,lun,'<h3>Density histogram in the high-res heat map.</h3>'
        printf,lun,'<img src="'+target_run+'-lrg-chunk'+chunk+'-lrg-denshist-maxres5.png"><br>'
...
pro bosstarget_util::run_plot_subreg_density_qso_chunk6
...
; always run this!
pro bosstarget_util::run_plot_subreg_density_qso, target_run, extra_name=extra_name
    ...
    for i=0L, n_elements(maxres_array)-1 do begin
        maxres=maxres_array[i]
        for j=0L,2 do begin
            enames=ename+['maskngc40','sgc40']
            if j eq 0 then begin
                delvarx, target_flags
                dmin=30
                dmax=80
                chunks=['ngcgood','sgc']
            endif else if j eq 1 then begin
                target_flags = ['qso_core_main']
                dmin=10
                dmax=70
                ; core is on a more restrictive mask
                chunks=['ngcgood2','sgc']
            endif else begin
                target_flags = ['qso_core_main','qso_bonus_main']
                dmin=30
                dmax=80
                chunks=['ngcgood','sgc']
            endelse

            for ci=0,n_elements(chunks)-1 do begin
                chunk = chunks[ci]

                self->plot_subreg_density_byrun, $
                    target_type, target_run, chunk, extra_name=enames[ci], $
                    /hammer, maxres=maxres, $
                    target_flags=target_flags, $
                    dmin=dmin,dmax=dmax
            endfor
            ;self->plot_subreg_density_byrun, $
            ;    target_type, target_run, 'sgc', extra_name=ename+'sgc40', $
            ;    /hammer, maxres=maxres, $
            ;    target_flags=target_flags,$
            ;    dmin=dmin,dmax=dmax
        endfor
...
pro bosstarget_util::run_plot_subreg_density_lrg, target_run
    ...
    for i=0L, n_elements(maxres_array)-1 do begin
        ...
        self->plot_subreg_density_byrun, $
            target_type, target_run, 'ngc', $
            /hammer, maxres=maxres
        self->plot_subreg_density_byrun, $
            target_type, target_run, 'sgc',  $
            /hammer, maxres=maxres
...
pro bosstarget_util::plot_subreg_density, ra, dec, chunks, psfile=psfile, ind=ind, $
		maxres=maxres, $
	...
	if n_elements(chunks) eq 1 then begin
		...
		if strn(chunks[0]) eq 'ngc' or strn(chunks[0]) eq 'ngcgood2' or strn(chunks[0]) eq 'ngcgood' then begin
	...
	if n_elements(dmin) eq 0 then begin
        dmin_use = min(ds.dstruct[w].density) 
    endif else begin
        dmin_use=dmin
    endelse
	if n_elements(dmax) eq 0 then begin
        dmax_use = max(ds.dstruct[w].density) 
    endif else begin
        dmax_use=dmax
    endelse
	...
	; over plot b=30
	if strn(chunks[0]) ne '1' and strn(chunks[0]) ne 'sgc' and strn(chunks[0]) ne '6' then begin
...
pro bosstarget_util::plot_subreg_density_byrun, target_type, target_run, chunks, $
		target_flags=target_flags, $
    ...
    if n_elements(maxres) eq 0 then maxres=5
    if n_elements(maxres) gt 1 then message,'maxres must be a scalar'

	if n_elements(target_flags) eq 0 then begin
		case target_type of 
			'qso': begin
				target_flags=$
					['qso_core_main','qso_bonus_main','qso_first_boss','qso_known_midz']
			end
			'lrg': begin
				target_flags = 'lrg'
			end
    ...
    if n_elements(dmin) eq 0 then begin
        if target_type eq 'qso' then dmin=30.0
        if target_type eq 'lrg' then dmin = 110.0
    endif
    if n_elements(dmax) eq 0 then begin
        if target_type eq 'qso' then dmax=80.0
        if target_type eq 'lrg' then dmax = 200.0
    endif
    ...
    if keyword_set(without_notflags) then psfile=[psfile,'nonot']
    psfile=strjoin(psfile,'-')+'.eps'
    psfile=path_join(psdir, psfile)
	...
	for i=0L, nchunk-1 do begin
		...
		w=self->btselect(str, target_flags, without_notflags=without_notflags)

		if n_elements(where_string) ne 0 then begin
			ww=self->_struct_select(str[w], where_string, nww,/verbose)
	...
	if strn(chunks[0]) eq '1' then begin
		ra=self->shiftra(ra,/wrap)
	endif
	...
	; Just the histogram
	psfile=[tr,target_type,fmid,'denshist']
    if n_elements(maxres) ne 0 then begin
        psfile=[psfile,'maxres'+ntostr(long(maxres))]
    endif
    if keyword_set(without_notflags) then psfile=[psfile,'nonot']
	...
	dens=ds.dstruct.density
    sigma_clip, dens, mn, sig
    xrange = mn + [-4.0*sig, 4.0*sig]
    ...
    if target_type eq 'lrg' then begin
        ...
        binsize=1.0
    endelse
    plothist, dens, xh,yh,bin=binsize, xtitle=textoidl('#/deg^2'), $
        xrange=xrange

	if n_elements(goal) ne 0 then begin
        maxy=max(yh)*10
		pplot, [goal,goal], [0,maxy], line=2, color='red', $
			/over
	endif
    mnstr = string(f='("mean: ",f0.2)',mn)
    sdevstr = string(f='("sdev: ",f0.2)',sig)
    legend, [strn(chunks),mnstr,sdevstr]

	endplot,/trim, /png;, dpi=55
...
pro bosstarget_util::do_heat_map, types
...
function bosstarget_util::sloane_tiles
	ntile = 9722
	tile_radius = 1.49
...
pro bosstarget_util::sloane_match, struct, mstruct, msloane, distances
	if n_elements(struct) eq 0 then begin
...
pro bosstarget_util::run_plothist_tile_density, target_type, target_run, qso=qso, lrg=lrg, anyresolve=anyresolve, fpobjc=fpobjc, noradcheck=noradcheck
...
pro bosstarget_util::plothist_tile_density, $
	...
	begplot, psfile, /encap, xsize=11, ysize=8.5, /color
	plot, self->shiftra(density_struct.ra,/wrap), density_struct.density, psym=8, $
...
pro bosstarget_util::plothist_sgc_tile_density, target_type, target_run, btflags, $
...
function bosstarget_util::sgc_lrg_tile_density_pars
	nihi=21
	nrlo=16
	ntri=21
	ndperp0=10
...
function bosstarget_util::sgc_qso_tile_density_pars, chi2_pars=chi2_pars, old=old
...
function bosstarget_util::qso_tile_density_file, target_run, qso_types, pars, extra_name
...
pro bosstarget_util::select_by_ra, str, minra, maxra, chunk, indices, tile_subset
	sra = self->shiftra(str.ra, /wrap)

	indices=where(sra ge minra and sra lt maxra, nw)


	tiles=self->bosstile_read(chunk=chunk)
	sracen = self->shiftra(tiles.ra,/wrap)
	rad = self->bosstile_radius()

	tile_subset=where( (sracen - rad) gt minra and (sracen+rad) lt maxra)
end

function bosstarget_util::_select_by_name, struct, cutname, count=count, $
		tile_subset=tile_subset

	sra = self->shiftra(struct.ra, /wrap)
	bq=obj_new('bosstarget_qso')
	pars=bq->pars()
...
pro bosstarget_util::get_kde_coadd_matches, target_run, struct, kde, tile_subset, $
...
pro bosstarget_util::tune_chunk2_ramod, outdir=outdir, goal=goal
...
pro bosstarget_util::tune_chunk2_ihi_cmass, outdir=outdir, goal=goal, num=num
...
pro bosstarget_util::contour_ihi_hiz_vs_ihi_cmass, target_run, chunk, n, $
...
pro bosstarget_util::contour_qso_chunk1_tile_density_lodens, target_run, $
		nvals=nvals, $
		door=door, $
...
pro bosstarget_util::compare_chunks_tune1d, $
...
pro bosstarget_util::tune_all1d, chunk, target_run, goal, str=str, extra_name=extra_name
...
pro bosstarget_util::tune_nn_value1d, chunk, target_run, goal, range=range, $
...
pro bosstarget_util::tune_mclike1d, chunk, target_run, goal, range=range, $
		str=str, denstruct=denstruct, number=number, $
		reselect=reselect, outdir=outdir, extra_name=extra_name, $
		noplot=noplot
...
pro bosstarget_util::tune_like1d, chunk, target_run, goal, range=range, $
		str=str, denstruct=denstruct, number=number, $
		reselect=reselect, outdir=outdir, extra_name=extra_name, $
		noplot=noplot
	...
	self->tune_chunk_density1d, $
		...
		reselect=reselect, $
		outdir=outdir, $
		xtitle='likelihood threshold v2', $
		noplot=noplot

	return
end

pro bosstarget_util::tune_nn1d, chunk, target_run, goal, range=range, $
		str=str, denstruct=denstruct, number=number, $
		reselect=reselect, outdir=outdir, extra_name=extra_name, $
		noplot=noplot
...
pro bosstarget_util::tune_kde1d, chunk, target_run, goal, range=range, $
		str=str, denstruct=denstruct, number=number, $
		reselect=reselect, outdir=outdir, extra_name=extra_name, $
		noplot=noplot
...
pro bosstarget_util::tune_chunk2_like, target_run, goal, str=str, denstruct=denstruct, n=n, $
...
pro bosstarget_util::tune_chunk2_nn, target_run, str=str, denstruct=denstruct, n=n, $
		reselect=reselect, outdir=outdir
	; note it's not really contour here since it is one dimensional
	; this is "commissioning 2, or comm2"
	; will look for about 20-25/square degree for both bright and faint?
...
pro bosstarget_util::tune_qsoed_core, str, chunk, denstruct=denstruct, number=number
    ; use bosstarget-qso-main008-edloose.fits

    ; use chunk ngcgood2 or ngclarge

    outdir='/clusterfs/riemann/raid008/bosswork/groups/boss/target/esheldon/qsoed-tune'
    target_flags = 'any'
	...
	threshtype='gt'
	goal=20.0
	tagname='qsoed_prob_core'
	range=[0.1,1.0]
	self->tune_chunk_density1d, $
		threshtype, chunk, 'qso', target_flags, target_run, tagname, range, goal, $
		str=str, $
		outdir=outdir, $
		xtitle='qsoed threshold', $
        number=number, $
        denstruct=denstruct
	return
end
...
pro bosstarget_util::tune_chunk_density1d, $
		threshtype, $
		chunk, $
		target_type, $
		target_flags, $
	...
	bt=obj_new('bosstarget')
	tilepoly=bt->chunk_polygon_read(chunk,/bounds)
    ...
    if target_flags ne 'any' then begin
        wtype=self->btselect(str, target_flags)
    endif else begin
        wtype = lindgen(n_elements(str))
    endelse
	...
	wtype2 = where( isin[wtype], ntype)
	wtype = wtype[wtype2]

    print,'Overall density in chunk: ',ntype/total_area
...
pro bosstarget_util::tune_qso_postcomm_loose, chunk, target_run, $
...
pro bosstarget_util::contour_chunk_bonus, chunk, target_run, num, $
		denstruct=denstruct
...
pro bosstarget_util::contour_chunk2_bonus_qsodens, target_run, str=str, $
...
pro bosstarget_util::plot_tuned_densities, str
	bq=obj_new('bosstarget_qso')
	...
	pplot, self->shiftra(ds.ra,/wrap), ds.density, psym=8, $
		xtitle='RA', ytitle='#/square degree'
	endplot,/landfix
end


pro bosstarget_util::tune_chunk1_qso_tile_density_kde_coadd_by_ra, $
...
pro bosstarget_util::tune_chunk2_qso_density_bonus_by_ra, target_run, $
...
pro bosstarget_util::tune_chunk1_tile_density_qso_kde_coadd_region, $
...
pro bosstarget_util::contour_chunk1_tile_density_qso_region, $
...
pro bosstarget_util::calculate_chunk1_tile_density_qso, $
...
pro bosstarget_util::calculate_chunk1_tile_density_qso_pbs, $
...
pro bosstarget_util::contour_chunk1_tile_density_qso, $
		target_run, qso_type, extra_name, $
		extra_cutname=extra_cutname, $
		commissioning=commissioning, $
...
pro bosstarget_util::calculate_sgc_tile_density_qso, $
		target_run, qso_types, pars, extra_name, $
		extra_cutname=extra_cutname, $
...
pro bosstarget_util::calculate_sgc_tile_density_qso_pbs, $
		target_run, qso_type, extra_name, $
		extra_cutname=extra_cutname, $
		walltime=walltime
...
pro bosstarget_util::contour_sgc_tile_density_qso, target_run, qso_type, extra_name, $
		density=density, bfratio=bfratio, generate=generate, struct=struct, $
...
pro bosstarget_util::calculate_sgc_tile_density_dperp0_ihi_pbs, target_run
...
pro bosstarget_util::calculate_sgc_tile_density_ihi_rlo_pbs, target_run, $
		noclobber=noclobber
...
pro bosstarget_util::calculate_sgc_tile_density_triangle_pbs, target_run, $
		noclobber=noclobber
...
pro bosstarget_util::contour_sgc_tile_densities_ihi_dperp0, target_run, $
		density=density
...
pro bosstarget_util::plot_sgc_tile_densities_cmass_ihi, struct=struct, generate=generate
...
pro bosstarget_util::contour_sgc_tile_densities_triangle, target_run, $
		density=density
...
pro bosstarget_util::contour_sgc_tile_densities_ihi_vs_rlo, target_run, $
		density=density
...
pro bosstarget_util::calculate_sgc_tile_density, target_type, target_run, btflags, $
		struct=struct, fpobjc=fpobjc, noradcheck=noradcheck, $
...
function bosstarget_util::tile_density_file, target_type, target_run, btflags, $
		fpobjc=fpobjc, noradcheck=noradcheck, $
...
function bosstarget_util::sloane_tile_density, struct,  $
...
pro bosstarget_util::make_run_html, tsstruct, target_type
...
function bosstarget_util::testbed_pars, runid
	case runid of
...
pro bosstarget_util::run_sgc_testbed, runid, _extra=_extra
...
pro bosstarget_util::collate_sgc_testbed, subdir
...
pro bosstarget_util::test_bounds_select, t, radius=radius
...
pro bosstarget_util::boss_bounds_plot, areaname=areaname
...
function bosstarget_util::boss_bounds_select, $
...
pro bosstarget_util::plot_qso_sgc_footprints, struct
...
pro bosstarget_util::plot_footprint, target_type, target_run, areaname, $
		extra_name=extra_name, $
		dorand=dorand, $
		fracuse=fracuse, $
		target_flags=target_flags, $
...
pro bosstarget_util::test2, all, sweeps, targets_notinsweeps 
...
function bosstarget_util::_struct_select, str, wstring, nw, verbose=verbose
...
pro bosstarget_util::test, fpobjc, sweep, fpobjc_gal_primary_target_flux,sweep_gal_primary_target_flux , cs=cs
...
pro bosstarget_util::setup_default
...
pro bosstarget_util::setup_sgc, reset=reset
	common sgc_common, $
...
pro bosstarget_util::make_sgc_targets, target_type, subdir, $
...
function bosstarget_util::sgc_target_dir, subdir
...
function bosstarget_util::sgc_collate_file, type, subdir, job=job, $
...
function bosstarget_util::sgc_collate_read, target_type, subdir, all=all, fpobjc=fpobjc
...
function bosstarget_util::qso_select_bypar, str, qso_types, pars, $
		count=count, usewhere=usewhere, res=res, $
		commissioning=commissioning
...
function bosstarget_util::qso_where_strings_bypar, pars_input, verbose=verbose
...
pro bosstarget_util::sgc_gather, target_type, target_run, all=all, fpobjc=fpobjc, runs=runs, typecut=typecut, anyresolve=anyresolve, outfile=outfile, target_dir=target_dir, match_method=match_method
...
function bosstarget_util::make_bayes_input, struct
	arrval=fltarr(5)
	larrval = lonarr(5)
	inst = {$
		psfcounts: arrval, $
...
pro bosstarget_util::run_bayes_prob, struct, probgal, probflags
...
pro bosstarget_util::sgc_test_target_probgal_runlist, runs, reruns
...
pro bosstarget_util::sgc_test_target_probgal, noclobber=noclobber, new=new
...
pro bosstarget_util::sgc_test_target_probgal_gather
...
pro bosstarget_util::sgc_test_plots, struct, psfile=psfile_input, leg=leg, $
...
pro bosstarget_util::_bayes_plots, struct, cmodeli, type
	!p.multi=[0,0,2]
...
pro bosstarget_util::sgc_test_target_probgal_plots, struct, cmodeli, new=new
...
pro bosstarget_util::gather_std
	bt=obj_new('bosstarget')
...
function bosstarget_util::splitlist, list, nsplit=nsplit, nper=nper
...
function bosstarget_util::riemann_nodeinfo
	return, {nnodes: 14, corespernode: 8, mempernode: 8}
end
pro bosstarget_util::sgc_runlist, runs, reruns, nsplit=nsplit, nper=nper
	self->setup_sgc
...
function bosstarget_util::sgc_bounds, padding=padding
...
function bosstarget_util::riemann_default_njobs
	return, 50
end
function bosstarget_util::sgc_extra_logic
...
pro bosstarget_util::make_partial_sgc_targets, target_type, target_run, jobnum, $
		nper=nper, $
		run_index=run_index, $
		all=all, $
		fpobjc=fpobjc, $
...
pro bosstarget_util::sgc_gather_partial, target_type, target_run, jobnum, nper=nper, $
...
function bosstarget_util::sgc_slow_runs
	runs = $
...
pro bosstarget_util::create_sgc_pbs, target_type, target_run, $
		dotarget=dotarget,$
		dogather=dogather,$
		sleep=sleep, $
		nper=nper, $
	...
	for job=0L, njobs-1 do begin
		...
		idl_commands = "eb=obj_new('bosstarget_util')"
...
function bosstarget_util::get_all_sweep_filenames, type
	bt=obj_new('bosstarget')
	bt->runlist, runs, reruns
...
pro bosstarget_util::test_new_qso,objs,oldres,newres
	bt=obj_new('bosstarget')
	bt->runlist, runs, reruns
...
pro bosstarget_util::compare_concentration, $
...
pro bosstarget_util::compare_concentration1755, $
		sweep, sweep_iconc, fpobjc_iconc
	; compare outputs from 
	;  standard gal datasweep run
	;  adding i concentration > 0.3 to datasweep run
...
pro bosstarget_util::test_bermuda_triangle, frac, struct=struct, mst=mst, $
...
pro bosstarget_util::lrg_cmodelmag_vs_dperp, struct, fracuse=fracuse, $
		psfile=psfile
...
pro bosstarget_util::compare_sweep_versions, sold, snew
...
pro bosstarget_util::plot_ukidss_colors, struct
	bq=obj_new('bosstarget_qso')
	lups = bq->get_lups(struct, /deredden)
...
pro bosstarget_util::write_all_sweep_radec
	bt=obj_new('bosstarget')
	bt->runlist, runs, reruns
...
pro bosstarget_util::trim_allpos_to_stripe82, limited=limited, t=t
	outdir=getenv('BOSS_TARGET')
	outdir=path_join(outdir,['esheldon','sgc_allpos'])
...
pro bosstarget_util::check_nn_densities, target_run, $
...
function bosstarget_util::bosstile_sub_regions_cache_file, chunks, maxres
...
function bosstarget_util::bosstile_sub_regions_cache_read, chunks, maxres
...
pro bosstarget_util::calculate_bosstile_sub_regions, $
		chunks=chunks, $
		geometry=geometry, $
		maxres=maxres, $
		$
	...
	if file_test(cache_file) and not keyword_set(force) then begin
        ...
        spawn,'rm /tmp/init.ply /tmp/join.ply /tmp/snap.ply '+$
            '/tmp/balkan.ply /tmp/pixel.ply'
	endelse
	...
	;colors= randomu(seed, long(max(regions.region))+1L)*!d.n_colors+40
    n=long(max(regions.region))+1L
	...
	if keyword_set(doplot) then begin
        ...
        color2=c2i("red")
        color3=c2i("blue")
		plot_poly, geometry, /fill, color=color1, offset=100.
		plot_poly, polygons, color=color, offset=100., /fill, /over
		if n_elements(init) ne 0 then begin
			plot_poly, init, out=1, color=color2, offset=100., /over
		endif
		plot_poly, polygons, out=3, color=color3, offset=100., /over
	endif

end


pro bosstarget_util::cache_bosstile_sub_regions, chunks, maxres
...
pro bosstarget_util::bosstile_sub_regions, chunks, maxres, $
...
function bosstarget_util::orcut_table, type=type
...
function bosstarget_util::orcut_logic, str, type=type, cuts=cuts
...
function bosstarget_util::orcut_select, str, nw, type=type, cuts=cuts
	logic = self->orcut_logic(str, cuts=cuts, type=type)
	w=where(logic, nw)
	return, w
end

function bosstarget_util::orcut_select_all, str, count, cuts=cuts, type=type
...
function bosstarget_util::fit_and_eval, x, y, degree, x2eval, yfit=yfit
...
pro bosstarget_util::orcut_plot, type, psfile=psfile, interpval=interpval
	cuts = self->orcut_table(type=type)
...
pro bosstarget_util::explore_orcuts, type, full=full
...
function bosstarget_util::gett, struct, name
	if strlowcase(name) eq 'ra' then begin
		c=self->shiftra(struct.ra, /wrap)
	endif else begin
...
pro bosstarget_util::compare_densities, $
		chunks, target_type, target_run, $
		reselect=reselect, $
		fixup=fixup, $
		extra_name=extra_name, $
...
pro bosstarget_util::plot_chunk_poly, chunks, bounds=bounds, offset=offset, $
...
function bosstarget_util::read_shirley_spec, matched=matched
...
pro bosstarget_util::plot_in_chunks, x, y, chunksize, over_index=over_index, $
...
pro bosstarget_util::plot_targets_and_tiles, $
...
function bosstarget_util::fstar_dist_cuts, nstep
	nstep=100
	cuts = arrscl(findgen(nstep), 0.04, 0.10)
	return, cuts
end
pro bosstarget_util::tune_fstar_std, struct=struct, density=density
...
pro bosstarget_util::mmt_match_fpobjc_pbs
	mmtdir = getenv("BOSSTARGET_DIR")
	mmtdir=path_join(mmtdir, "data")
...
pro bosstarget_util::compare_like_new, str=str
	bt=obj_new('bosstarget')
...
pro bosstarget_util::boss1_test, targs, lrg_intermed, qso_intermed, lrg, qso, std
	...
	symsize=0.15
	pplot, self->shiftra(lrg.ra,/wrap), lrg.dec, psym=8, symsize=symsize, $
		xtit=xtit, ytit=ytit,$
		yrange=[-1.5,2], ystyle=3
	pplot, self->shiftra(lrg_intermed.ra,/wrap), lrg_intermed.dec, /over, $
	...
	symsize=0.25
	pplot, self->shiftra(qso.ra,/wrap), qso.dec, psym=8, symsize=symsize, $
		xtit=xtit, ytit=ytit,$
		yrange=[-1.5,2], ystyle=3
	pplot, self->shiftra(qso_intermed.ra,/wrap), qso_intermed.dec, /over, $
		psym=8, symsize=symsize, $
		color='red'
	...
	pplot, self->shiftra(lrg.ra,/wrap), lrg.dec, psym=8, symsize=symsize, $
		xtit=xtit, ytit=ytit,$
		yrange=[-1.5,2], ystyle=3
	pplot, self->shiftra(targs[wlrg].ra,/wrap), targs[wlrg].dec, /over, $
		psym=8, symsize=symsize, $
		color='red'
	...
	pplot, self->shiftra(qso.ra,/wrap), qso.dec, psym=8, symsize=symsize, $
		xtit=xtit, ytit=ytit,$
		yrange=[-1.5,2], ystyle=3
	pplot, self->shiftra(targs[wqso].ra,/wrap), targs[wqso].dec, /over, $
		psym=8, symsize=symsize, $
		color='red'
...
function bosstarget_util::_plot_poly_ra, ra, offset=offset, wrap=wrap

    if n_elements(offset) ne 0 then begin
        ramoffset=(ra-offset) 
        indx=where(ramoffset lt 0.,count)
        if(count gt 0) then ramoffset[indx]=ramoffset[indx]+360.
    endif else if keyword_set(wrap) then begin
        ramoffset = self->shiftra(ra, /wrap)
    endif else begin
        ramoffset=ra
    endelse
...
pro bosstarget_util::plot_poly,poly,wrap=wrap, offset=offset,xrange=xrange,yrange=yrange, $
        ytitle=ytitle, $
        ...
        for i=0L, n_elements(poly)-1L do begin
            if(garea(poly[i]) gt 0.) then begin
                ...
                if(keyword_set(verts)) then begin
                    ...
                    ramoffset=self->_plot_poly_ra(ra, offset=offset, wrap=wrap)
    ...
    if(NOT keyword_set(over)) then begin
        if n_elements(ytitle) eq 0 then ytitle='DEC'
        if keyword_set(splot) then begin
            splot,[0],[0],/nodata,xtitle=xtitle,ytitle=ytitle, $
...
pro bosstarget_util::plot_boss_poly, two=two, epsfile=epsfile, color=color
    bt=obj_new('bosstarget')


    if keyword_set(two) then begin
        if n_elements(epsfile) ne 0 then begin
            begplot,epsfile,color=color,xsize=11.5,ysize=8.5,/encap
        endif

        ; separate plots for ngc and sgc
        ngc = bt->chunk_polygon_read('ngcgood2')
        sgc = bt->chunk_polygon_read('sgc')

        erase & multiplot, [2,1], /square, xgap=0.03

        self->plot_poly, ngc, /fill,xstyle=3,ystyle=3,$
            xrange=[100,275],yrange=[-10,80]
        legend,/right,'NGC'
        multiplot
        self->plot_poly, sgc, offset=90, /fill,xstyle=3,ystyle=3, $
            xrange=[220,325], yrange=[-15,45], ytitle=''
        legend,/right,'SGC'
        multiplot,/default
    endif else begin
        if n_elements(epsfile) ne 0 then begin
            begplot,epsfile,color=color,xsize=11.5,ysize=7.5,/encap
        endif
        ply = bt->chunk_polygon_read('boss')
        self->plot_poly, ply, /fill,xstyle=3,ystyle=3,/wrap, $
            xrange=[-70,300], yrange=[-20,80], $
            charsize=2, xtitle='RA [degrees]', ytitle='DEC [degrees]'
    endelse
...
pro bosstarget_util::compare_jessica
...
function bosstarget_util::shiftra, ra, ra_shift, wrap=wrap
	if n_elements(ra) eq 0 or $
			(not keyword_set(wrap) and n_elements(ra_shift) eq 0) then begin
		on_error,2
		print,'Usage: new_ra = self->shiftra(ra, shift, /wrap)'
		message,'Halting'
	endif

	if keyword_set(wrap) then begin
		ranew = ra
		w=where(ra gt 300, nw)
		if nw ne 0 then begin
			ranew[w] -= 360
		endif
	endif else begin
...
function bosstarget_util::split_by_semicolon, list
	ls = strsplit(list, ';', /extract)
...
pro bosstarget_util::select_2010_07, orig=orig

    target_run = 'vcat-2010-07-02'

    dir=getenv('BOSS_TARGET')
    dir=filepath(root=dir,target_run)
    plotdir=filepath(root=dir,'plots')

    orig_file=filepath(root=dir,$
            'star82-varcat-bound-ts-wgalex-wnnvarbonus-z2pt2-galexbitset.fits.gz')
    if n_elements(orig) eq 0 then begin
        print,'reading original file:',orig_file
        orig=mrdfits(orig_file,1)
    endif
    ...
    new_file = bt->target_file('qso',target_run,/collate, target_dir=dir)

    ws=bt->default_where_string('qso',/anyresolve)
    keep = bt->where_select(orig, ws, nkeep)
    splog,nkeep,norig,format='("Keeping: ",i0,"/",i0)'


    status_struct = bt->status_struct(/min)

    status_struct.photoop_v = photoop_version()
    status_struct.bosstarget_v = bosstarget_version()
    status_struct.idlutils_v = idlutils_version()
    status_struct.photo_sweep = file_basename(getenv('PHOTO_SWEEP'))
    status_struct.photo_resolve = file_basename(getenv('PHOTO_RESOLVE'))
    status_struct.photo_calib = file_basename(getenv('PHOTO_CALIB'))

    status_struct.target_type = 'qso'
    status_struct.target_run = target_run

    new = orig[keep]
    print,'writing file: ',new_file
    mwrfits, status_struct, new_file, /create
    mwrfits, new, new_file

end


; some qa for the varcats selection created july 2010

pro bosstarget_util::qa_2010_07, str=str

    dir=getenv('BOSS_TARGET')
    dir=filepath(root=dir,'varcats-2010-07')
    plotdir=filepath(root=dir,'plots')

    if n_elements(str) eq 0 then begin
          file=filepath(root=dir,$
            'star82-varcat-2010-07-02.fits')
        print,'reading: ',file
        str=mrdfits(file,1)
    endif
    ...
    flags=$
        ...
        'qso_core_main',$
        'qso_bonus_main']
    f=sdss_flagval('boss_target1',flags)
    wall=where( (str.boss_target1 and f) ne 0)

    wboss = where(bt->is_in_chunk('boss',str[wall].ra,str[wall].dec), nboss)
    wboss=wall[wboss]

    radecps = filepath(root=plotdir, 'radec.eps')
    begplot,radecps,/color,xsize=11,ysize=8.5

    ra=self->shiftra(str.ra, /wrap)

    plot, ra[wall], str[wall].dec, psym=8, symsize=0.2, $
        xtitle='RA (wrap)', ytitle='DEC'
    pplot, ra[wboss], str[wboss].dec, psym=8, $
        symsize=0.2, $
        /overplot, color='blue'

    plegend, ['All','In BOSS'], psym=8, color=[!p.color,c2i('blue')], $
        /right
    endplot,/trim,/png


    wb = where( (str[wboss].boss_target1 and sdss_flagval('boss_target1','qso_bonus_main')) ne 0, nbonus)
    wb=wboss[wb]
    wk = where( (str[wboss].boss_target1 and sdss_flagval('boss_target1','qso_known_midz')) ne 0, nknown)
    wk=wboss[wk]
    wf = where( (str[wboss].boss_target1 and sdss_flagval('boss_target1','qso_first_boss')) ne 0, nfirst)
    wf=wboss[wf]

    bin=5.0
    area = 5.0*(1.25+1.25)

    bs = binner(ra[wboss], bin=bin)
    bsb = binner(ra[wb], bin=bin)
    bsk = binner(ra[wk], bin=bin)
    bsf = binner(ra[wf], bin=bin)



    radecps = filepath(root=plotdir, 'radens.eps')
    begplot,radecps,/color,xsize=11,ysize=8.5
    colors=[!p.color,c2i('blue'),c2i('magenta'),c2i('red')]

    plot, bs.xcenter, bs.hist/area, psym=10, $
        xtitle='RA (wrap)', ytitle=textoidl('Density (# deg^{-2})'), $
        xrange=[-60,60], xstyle=3
    pplot, bsb.xcenter, bsb.hist/area, psym=10, color=colors[1],/over
    pplot, bsk.xcenter, bsk.hist/area, psym=10, color=colors[2],/over
    pplot, bsf.xcenter, bsf.hist/area, psym=10, color=colors[3],/over

    plegend,$
        ['All   ('+strn(nboss)+')',$
         'Bonus ('+strn(nbonus)+')',$
         'Known midz ('+strn(nknown)+')',$
         'First ('+strn(nfirst)+')'],$
             line=0,color=colors,/right
    endplot,/trim,/png


end

pro bosstarget_util::process_varcats_ukidss
    bq=obj_new('bosstarget_qso')

    dir=getenv('BOSS_TARGET')
    if dir eq '' then message,'BOSS_TARGET is not set'
    infile = filepath(root=dir, sub='myers-varcat', 'star82-varcat-bound-ts-se-nncomb-ADMstripe82.fits.gz')
    ...
    st = bq->struct()
    addst = {psfflux:fltarr(5), psfflux_ivar:fltarr(5), extinction:fltarr(5)}
    st = create_struct(st, addst)
    st = replicate(st, nobjs)
	struct_assign, objs, st, /nozero



    edfile=filepath(root=dir, sub='myers-varcat', 'qsoed-cache.fits')
    if not file_test(edfile) then begin
        splog,'Cacheing ed: ',edfile
        ; magnitude range
        gmag_bitmask = bq->gmag_logic(objs)

        ;ADM need this for FIRST (don't need to cut bright FIRST objects only faint)
        gmag_bitmask_nob = bq->gmag_logic(objs, /no_bright_cut)
        resolve_bitmask = bq->resolve_logic(objs)

        bitmask = gmag_bitmask+resolve_bitmask

        ed_struct_bonus = bq->run_qsoed(objs, bitmask=bitmask, /verbose, /multi)

        splog,'Writing: ',edfile
        mwrfits, ed_struct_bonus, edfile, /create
    endif
    splog,'Reading ed cache: ',edfile
    ed_struct_bonus = mrdfits(edfile,1)

    st.qsoed_prob_bonus = ed_struct_bonus.pqso
    ; had bug where was still copied even if flux was 
    ; missing. this is fixed in latest run_qsoed
    w=where(ed_struct_bonus.ukidssmatch ne 0, nw)
    if nw gt 0 then st[w].qsoed_prob_bonus_ukidss = ed_struct_bonus[w].pqso_ukidss
    st.galex_match = ed_struct_bonus.galexmatch
    st.ukidss_match = ed_struct_bonus.ukidssmatch
    ...
    outfile=filepath(root=dir, sub='myers-varcat', 'test-ukidss.fits')
    splog,'Writing: ',outfile
    mwrfits, st, 'test-ukidss.fits', /create
end

function bosstarget_util::tmpfile, tmpdir=tmpdir, prefix=prefix, suffix=suffix
    ; copied from sdssidl

...
;                   - Bovy
;-
PRO PREP_DATA, flux, flux_ivar, extinction=extinction, $
               mags=mags, var_mags=var_mags, $
               colors=colors, nbyncovar=nbyncovar, fluxes=fluxes, $
               relfluxes=relfluxes, bs=bs
_BIGVAR= 1D5
...
if keyword_set(extinction) THEN BEGIN
    flux= sdss_deredden(flux,extinction)
    flux_ivar= sdss_deredden_error(flux_ivar,extinction)
ENDIF
...
FOR ii=5L, nfluxes-1 DO mags[ii,*]= flux2mags(flux[ii,*])
IF keyword_set(nbyncovar) THEN BEGIN
    FOR ii=0L, 4 DO var_mags[ii,ii,*]= sdss_ivar2magerr(flux_ivar[ii,*],flux[ii,*],bs[ii])^2D0
    FOR ii=5L, nfluxes-1 DO var_mags[ii,ii,*]= ivar2magerr(flux_ivar[ii,*],flux[ii,*],bs[ii])^2D0
ENDIF ELSE BEGIN
    FOR ii=0L, 4 DO var_mags[ii,*]= sdss_ivar2magerr(flux_ivar[ii,*],flux[ii,*],bs[ii])^2D0
    FOR ii=5L, nfluxes-1 DO var_mags[ii,*]= ivar2magerr(flux_ivar[ii,*],flux[ii,*])^2D0
ENDELSE

...
;      xdqsoz_plotfracs
;   PURPOSE:
;      plot the fraction of the PDF < z_spec
;   INPUT:
...
;-
PRO XDQSOZ_PLOTFRACS, plotfile, tmpfile=tmpfile, $
...
IF ~file_test(tmpfile) THEN BEGIN
    ...
    for ii=0L, nqso-1 do begin
        ...
        qsolike= marginalize_colorzprob(0.3,qso[ii].z,$
                                        flux[*,ii],$
                                        flux_ivar[*,ii],$
                                        galex=galex,ukidss=ukidss,norm=allqsolike)
        frac[ii]= qsolike/allqsolike
    endfor
    save, filename=tmpfile, frac, mi
ENDIF ELSE BEGIN
    restore, filename=tmpfile
ENDELSE
k_print, filename=plotfile
hogg_plothist, frac, xrange=[0.,1.], xtitle='fraction of p(z) < z_{spec}'
k_end_print
END

...
;-
;------------------------------------------------------------------------------
function likelihood_everything_file_v2, lfile, area=area
...
function likelihood_vector_v2, objs_in, objs_cat, area
...
function _compile_qsolike_v2, recompile=recompile
	common likelihood_vector_fast_block_v2, name, srcdir, extra_cflags
...
function likelihood_vector_fast_v2, objs, model, area, recompile=recompile
	...
	; shared object
	dll_path = _compile_qsolike_v2(recompile=recompile)
	res = call_external( $
		dll_path, name, $
		float(area), $
		nmodel, $
...
;------------------------------------------------------------------------------
pro likelihood_compute_v2, objs_in1, objs_out, adderr=adderr, $
		fast=fast, recompile=recompile

	common likelihood_compute_block_v2, objs1, area1, objs2, area2
   ...
   if n_elements(objs1) eq 0 then begin
	   ...
	   objs1 = likelihood_everything_file_v2(file1, area=area1)
   ...
   if n_elements(objs2) eq 0 then begin
	   ...
	   file2 = filepath('Likeli_QSO_V2.fits.gz', $
		root_dir=getenv('BOSSTARGET_DIR'), subdir='data')
   ...
   nzbin = 45
   dz = 0.1
   zmin = 0.5 + dz * lindgen(nzbin)
   zmax = 0.5 + dz * (lindgen(nzbin)+1)
   ...
   if keyword_set(fast) then begin
      if (ct1 GT 0) then objs_out.l_everything_array[0] = $
        likelihood_vector_fast_v2(objs_in, objs1[i1], area1, $
		                          recompile=recompile)
      if (ct2 GT 0) then objs_out.l_everything_array[1] = $
        likelihood_vector_fast_v2(objs_in, objs1[i2], area1, $
	                              recompile=recompile)
      if (ct3 GT 0) then objs_out.l_everything_array[2] = $
        likelihood_vector_fast_v2(objs_in, objs1[i3], area1, $
		                          recompile=recompile)
      if (ct4 GT 0) then objs_out.l_everything_array[3] = $
        likelihood_vector_fast_v2(objs_in, objs1[i4], area1, $
		                          recompile=recompile)
      if (ct5 GT 0) then objs_out.l_everything_array[4] = $
        likelihood_vector_fast_v2(objs_in, objs1[i5], area1, $
		                          recompile=recompile)
      if (ct6 GT 0) then objs_out.l_everything_array[5] = $
        likelihood_vector_fast_v2(objs_in, objs1[i6], area1, $
		                          recompile=recompile)
      if (ct7 GT 0) then objs_out.l_everything_array[6] = $
        likelihood_vector_fast_v2(objs_in, objs1[i7], area1, $
		                          recompile=recompile)
   endif else begin
      if (ct1 GT 0) then objs_out.l_everything_array[0] = $
       likelihood_vector_v2(objs_in, objs1[i1], area1)
      if (ct2 GT 0) then objs_out.l_everything_array[1] = $
       likelihood_vector_v2(objs_in, objs1[i2], area1)
      if (ct3 GT 0) then objs_out.l_everything_array[2] = $
       likelihood_vector_v2(objs_in, objs1[i3], area1)
      if (ct4 GT 0) then objs_out.l_everything_array[3] = $
       likelihood_vector_v2(objs_in, objs1[i4], area1)
      if (ct5 GT 0) then objs_out.l_everything_array[4] = $
       likelihood_vector_v2(objs_in, objs1[i5], area1)
      if (ct6 GT 0) then objs_out.l_everything_array[5] = $
       likelihood_vector_v2(objs_in, objs1[i6], area1)
      if (ct7 GT 0) then objs_out.l_everything_array[6] = $
       likelihood_vector_v2(objs_in, objs1[i7], area1)
   endelse
   ...
   for ibin=0, nzbin-1 do begin
	  ...
	  if keyword_set(fast) then begin
		  objs_out.l_qso_z[ibin] = $
			  likelihood_vector_fast_v2(objs_in, objs2[indx], area2)
	  endif else begin
		  objs_out.l_qso_z[ibin] = $
			  likelihood_vector_v2(objs_in, objs2[indx], area2)
	  endelse
   endfor

   eps = 1e-30
   ;Quasars in BOSS redshift range - between redshift 2.1 and 3.7
   objs_out.l_qso_boss = total(objs_out.L_QSO_Z[16:32],1) 
   qsolcut1 = where(alog10(objs_out.l_qso_boss) LT -9.0,qsolcut1cnt)
   ;ADM added the qsolcut1cnt and the if statement that uses it, below
   ;ADM so the code doesn't die on small or incomplete runs
   den = total(objs_out.L_EVERYTHING_ARRAY[0:4],1) + total(objs_out.L_QSO_Z[0:44],1) + eps
   num = objs_out.l_qso_boss + eps
   objs_out.l_ratio = num/den
   if qsolcut1cnt gt 0 then begin
	   objs_out[qsolcut1].l_ratio = 0 ; eliminate objects with low L_QSO value
   endif
